"""Tests for linkit.client (LinkitClient facade)."""
...
class TestLoginFromBrowser:
    async def test_chrome_proxy_path(self, tmp_path):
        ...
        with patch("linkit.client.extract_cookies_from_browser", new_callable=AsyncMock) as mock_extract:
    ...
    async def test_invalid_cookies_raises(self, tmp_path):
        ...
        with patch("linkit.client.extract_cookies_from_browser", new_callable=AsyncMock), \
             patch("linkit.client.validate_session", new_callable=AsyncMock, return_value=False):
...
class TestLoginWithCookies:
    async def test_success(self, tmp_path):
        ...
        with patch("linkit.client.login_with_cookies", new_callable=AsyncMock, side_effect=fake_login), \
             patch("linkit.client.validate_session", new_callable=AsyncMock, return_value=True):
    ...
    async def test_invalid_raises(self, tmp_path):
        ...
        with patch("linkit.client.login_with_cookies", new_callable=AsyncMock), \
             patch("linkit.client.validate_session", new_callable=AsyncMock, return_value=False):
            with pytest.raises(AuthError, match="invalid or expired"):
...
class TestLoginFromSaved:
    async def test_success(self, tmp_path):
        ...
        with patch("linkit.client.validate_session", new_callable=AsyncMock, return_value=True):
...
class TestDelegation:
    ...
    async def test_get_my_posts(self, tmp_path):
        client = LinkitClient(cookies_path=str(tmp_path / "c.json"), timezone="UTC", timezone_offset=0)
        posts = [Post(urn="urn:1", text="hi")]
        with patch("linkit.feed.get_my_posts", new_callable=AsyncMock, return_value=posts):
    ...
    async def test_search_posts(self, tmp_path):
        ...
        with patch("linkit.search.search_posts", new_callable=AsyncMock, return_value=posts):
    ...
    async def test_get_feed(self, tmp_path):
        ...
        with patch("linkit.feed.get_feed", new_callable=AsyncMock, return_value=posts):
            result = await client.get_feed(limit=20)
        assert result == posts

    async def test_create_post(self, tmp_path):
        client = LinkitClient(cookies_path=str(tmp_path / "c.json"), timezone="UTC", timezone_offset=0)
        with patch("linkit.poster.create_post", new_callable=AsyncMock, return_value="urn:li:share:1"):
            urn = await client.create_post("Hello")
        assert urn == "urn:li:share:1"

    async def test_delete_post(self, tmp_path):
        client = LinkitClient(cookies_path=str(tmp_path / "c.json"), timezone="UTC", timezone_offset=0)
        with patch("linkit.poster.delete_post", new_callable=AsyncMock):
            await client.delete_post("urn:li:share:1")

    async def test_repost(self, tmp_path):
        client = LinkitClient(cookies_path=str(tmp_path / "c.json"), timezone="UTC", timezone_offset=0)
        with patch("linkit.poster.repost", new_callable=AsyncMock, return_value="urn:li:share:r1"):
            urn = await client.repost("urn:li:share:orig", text="Nice!")
        assert urn == "urn:li:share:r1"

    async def test_upload_image(self, tmp_path):
        client = LinkitClient(cookies_path=str(tmp_path / "c.json"), timezone="UTC", timezone_offset=0)
        with patch("linkit.media.upload_image", new_callable=AsyncMock, return_value="urn:li:media:1"):
    ...
    async def test_create_post_with_image(self, tmp_path):
        client = LinkitClient(cookies_path=str(tmp_path / "c.json"), timezone="UTC", timezone_offset=0)
        with patch("linkit.media.upload_image", new_callable=AsyncMock, return_value="urn:li:media:1"), \
             patch("linkit.poster.create_post_with_media", new_callable=AsyncMock, return_value="urn:li:share:img1"):
    ...
    async def test_create_scheduled_post(self, tmp_path):
        ...
        with patch("linkit.poster.create_scheduled_post", new_callable=AsyncMock, return_value="urn:li:share:sched1"):
